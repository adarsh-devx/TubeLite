...
class YtDlpPlugin(private val activity: Activity) : Plugin(activity) {
    ...
    .addOption("--verbose")
                ...
                Log.d(TAG, "[ANDROID YTDLP] exitCode=${response.exitCode} success=${response.isSuccess}")
                Log.d(TAG, "[ANDROID YTDLP] stdout length: ${jsonOutput.length}")
                Log.d(TAG, "[ANDROID YTDLP] stderr length: ${stderrOutput.length}")
                Log.d(TAG, "[ANDROID YTDLP] stderr preview: ${stderrOutput.take(2000)}")

                if (!response.isSuccess) {
                    invoke.reject("yt-dlp failed (exit ${response.exitCode}): ${stderrOutput.take(500)}")
                    return@launch
                }

                val result = parseYtdlpJson(jsonOutput)
                if (result != null) {
                    Log.d(TAG, "[ANDROID YTDLP] Parsed title: ${result.optString("title", "unknown")}")
                    Log.d(TAG, "[ANDROID YTDLP] video_formats count: ${result.optJSONArray("video_formats")?.length() ?: 0}")
                    Log.d(TAG, "[ANDROID YTDLP] audio_formats count: ${result.optJSONArray("audio_formats")?.length() ?: 0}")
    ...
    fun openFile(invoke: Invoke) {
        ...
        try {
            ...
            Log.d(TAG, "[OPEN_FILE] contentUri=$contentUri mimeType=$mimeType")
    ...
    private fun parseYtdlpJson(json: String): JSObject? {
        return try {
            ...
            val result = JSObject()
            result.put("title", obj.optString("title", "Unknown Title"))
            result.put(
                "channel",
                obj.optString("channel", obj.optString("uploader", "Unknown Channel"))
            )
            result.put("thumbnail", obj.optString("thumbnail", ""))

            val durationSec = obj.optDouble("duration", 0.0)
            result.put("duration", formatDuration(durationSec))
            ...
            if (formatsArray != null) {
                ...
                for (i in 0 until formatsArray.length()) {
                    ...
                    val actualSize = if (filesize > 0) filesize else filesizeApprox
                    ...
                    if (hasVideo && height > 0 && ext == "mp4") {
                        ...
                        if (!seenQualities.contains(quality)) {
                            ...
                            vf.put("label", "${qualityLabel(height)} · ${formatFilesize(actualSize)}")
                            vf.put("format", "MP4")
                            vf.put("size", formatFilesize(actualSize))
                            videoFormats.add(vf)
                        }
                    } else if (hasAudio && !hasVideo && abr > 0) {
                        val af = JSObject()
                        af.put("quality", "${ext.uppercase()} ${abr.toInt()}")
                        af.put("label", "${if (abr >= 256) "High Quality" else "Standard"} · ${formatFilesize(actualSize)}")
                        af.put("bitrate", "${abr.toInt()} kbps")
                        af.put("size", formatFilesize(actualSize))
                        audioFormats.add(af)
                    }
                }

                videoFormats.sortByDescending {
                    it.optString("quality", "0p").replace("p", "").toIntOrNull() ?: 0
                }
                audioFormats.sortByDescending {
                    it.optString("bitrate", "0 kbps").replace(" kbps", "").toIntOrNull() ?: 0
                }
            }

            result.put("video_formats", videoFormats.toTypedArray())
            result.put("audio_formats", audioFormats.toTypedArray())
            result
        } catch (e: Exception) {
